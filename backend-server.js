...
// Configuración de base de datos para Render.com
...
// ... (código existente antes del endpoint)

// Obtener información de un pedido (asegurar que devuelva geometría)
    ...
    (err, row) => {
      ...
      try {
        const pedido = {
          ...
          geometria_ruta: row.geometria_ruta  // Mantener geometría
        };
        res.json(pedido);
      } catch (parseErr) {
...
app.post('/api/pedidos/en-camino', (req, res) => {
  try {
    ...
    db.run(
      ...
      function(err) {
        if (err) {
          console.error('Error al actualizar pedido a en camino:', err);
...
app.post('/api/pedidos/entregado', (req, res) => {
  try {
    ...
    console.error('Error inesperado en finalización de pedido:', error);
    res.status(500).json({ error: 'Error interno del servidor' });
  }
});

// Verificar ubicación y cambiar estado
app.post('/api/pedidos/verificar-ubicacion', async (req, res) => {
  try {
    const { pedidoId, lat, lng } = req.body;
    const MARGEN_ERROR = 0.001; // ~100 metros
    ...
    if (distanciaVenta <= MARGEN_ERROR && pedido.estado === 'asignado') {
