    ...
    db.run(query, params, (err) => {
      if (err) {
        ...
        if (err.message.includes('UNIQUE constraint failed')) {
          if (err.message.includes('carnet')) {
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Limpiar teléfono (remover caracteres no numéricos)
    const telefonoLimpio = identificador.replace(/\D/g, '');
    ...
    const query = `
      ...
      ) WHERE (telefono = ? OR correo = ? OR telefono = ?) AND rol = ?
    `;
    
    db.get(query, [telefonoLimpio, identificador, identificador, rol], async (err, row) => {
      if (err) {
