...
// Endpoint para asignar mensajero a pedido (eliminado estado "asignado")
...
// Endpoint para verificar ubicación con cálculo de distancia preciso
app.post('/api/pedidos/verificar-ubicacion', async (req, res) => {
  try {
    const { pedidoId, lat, lng } = req.body;
    const MARGEN_METROS = 30; // 30 metros
    ...
    // Cambiar estado a 'en camino' si está cerca del punto de venta
    if (distanciaVenta <= MARGEN_METROS && pedido.estado === 'en proceso') {
      nuevoEstado = 'en camino';
    } 
    // Cambiar estado a 'entregado' si está cerca del punto de entrega
    else if (distanciaEntrega <= MARGEN_METROS && pedido.estado === 'en camino') {
