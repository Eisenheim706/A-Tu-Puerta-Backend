...
// Solicitar código de verificación (MEJORADO)
app.post('/api/mensajeros/solicitar-codigo', (req, res) => {
  try {
    ...
    db.run(query, [telefono, codigo, expiracion], (err) => {
      ...
      res.json({ success: true, codigo: codigo }); // Devuelve el código para desarrollo
    });
  } catch (error) {
...
// Verificar código de acceso (MEJORADO)
...
// Manejo de rutas no encontradas (IMPORTANTE)
app.use((req, res) => {
